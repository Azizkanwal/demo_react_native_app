...
import {getSendDailyStatusReports, startLoader, stopLoader} from "../../redux/slices";
import {DailyStatusList} from "./DailyStatusList";
import Toast from "react-native-toast-message";

export const SendDsrScreen = (props: any) => {
    const {navigation, route} = props;
    const [loading, setLoading] = useState(false);
    const [search, setSearch] = useState("");
    const itemPerPage = 10;
    const dispatch = useAppDispatch();
    const {rows, count, currentPage} = useAppSelector((state) => state.statusReport.sendDailyStatusReports);
    ...
    const onPageChange = async (page) => {
        ...
        await dispatch(getSendDailyStatusReports({page: page + 1, items: itemPerPage},renderResponseMessage));
            ...
            <ScrollView bounces={false}>
                <View style={styles.containerView}>
                    <Text style={styles.dailyStatusText}>
                        Daily Status Send List
                    </Text>
                </View>
                <View style={styles.dailyStatusList}>
                    <DailyStatusList type={'send'}
