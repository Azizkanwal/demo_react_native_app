...
export const AddDSR = (props: any) => {
    ...
    const newTask = {
        task: '', start_time: null, end_time: null, description: '', projectKey: 1,
        errors: {
            projectError: false,
            taskError: false,
    ...
    const newProject = {...newTask, new_project: true, project_id: '', new_task: true};
    ...
    const sendDSR = async () => {
        ...
        if (!validate()) {
            ...
            await reportDetailAttributes.map(({
                                                  new_project,
                                                  project_id,
                                                  task,
                                                  description,
                ...
                formData.append(`daily_status_reports_attributes[${index}][task]`, task);
                formData.append(`daily_status_reports_attributes[${index}][description]`, description);
                formData.append(`daily_status_reports_attributes[${index}][start_time]`, start_time.toString());
                formData.append(`daily_status_reports_attributes[${index}][end_time]`, end_time.toString());
                if (new_project) {
                    formData.append(`daily_status_reports_attributes[${index}][project_id]`, project_id);
                }
            });

            toIds && toIds.map((id) => formData.append('to_ids[]', id));
            ccIds && ccIds.map((id) => formData.append('cc_ids[]', id));
            await dispatch(submitDailyStatusReport(formData, (success, response) => {
                if (success) {
                    navigation.navigate('DsrDetail', {id: response.data.id, type: 'send'});
                }
    ...
    const validate = () => {
        ...
        reportDetailAttributeData.map((data) => {
            if (data.new_project && data.project_id === '') {
                data.errors = {...data.errors, projectError: true};
                error = true;
            }
            if (data.task.trim() === '') {
                data.errors = {...data.errors, taskError: true};
                error = true;
            }
            ...
            if (!data.start_time) {
                data.errors = {...data.errors, startTimeError: true};
                error = true;
            }
            if (!data.end_time) {
                data.errors = {...data.errors, endTimeError: true};
                error = true;
            }
        });
    ...
    const addNewTask = (projectKey) => {
        let reportDetailAttributeData = reportDetailAttributes;
        let index = 0;
        reportDetailAttributes.map((project, indexValue) => {
            if (project.projectKey === projectKey) {
                index = indexValue + 1;
            }
        });
        reportDetailAttributes.splice(index, 0, {...newTask, projectKey: projectKey});
        setReportDetailAttributes(reportDetailAttributeData);
        setEventCount(eventCount + 1);
    };

    const addNewDuplicateTask = (projectKey, oldIndex) => {
        let reportDetailAttributeData = reportDetailAttributes;
        let oldTask = reportDetailAttributeData[oldIndex];
        let taskData = newTask;
        if (oldTask) {
            let {new_project, project_id, new_task, ...restData} = oldTask;
            taskData = restData;
        }
        let index = 0;
        reportDetailAttributes.map((project, indexValue) => {
        ...
        reportDetailAttributes.splice(index, 0, {...taskData, projectKey: projectKey});
        setReportDetailAttributes(reportDetailAttributeData);
        setEventCount(eventCount + 1);
    };

    const removeTask = (index) => {
        let reportDetailAttributeData = reportDetailAttributes.filter((project, i) => i !== index);
        setReportDetailAttributes(reportDetailAttributeData);
        setEventCount(eventCount + 1);
    };
    ...
    const setTask = (task: string, index: number) => {
        let data = fetchRecordByIndex(index);
        let newReportDetailAttributes = reportDetailAttributes;
        if (task === ' ') {
            task = '';
        }
        newReportDetailAttributes[index] = {...data, task: task};
        setReportDetailAttributes(newReportDetailAttributes);
        clearErrorMessage('taskError', index);
        setEventCount(eventCount + 1);
    };
    ...
    const setTimeStart = (time: Date, index: number) => {
        let data = fetchRecordByIndex(index);
        let newReportDetailAttributes = reportDetailAttributes;
        newReportDetailAttributes[index] = {...data, start_time: time};
        setReportDetailAttributes(newReportDetailAttributes);
        setEventCount(eventCount + 1);
        clearErrorMessage('startTimeError', index);
        checkTimeDifference(newReportDetailAttributes, index);
    };

    const setTimeEnd = (time: Date, index: number) => {
        let data = fetchRecordByIndex(index);
        let newReportDetailAttributes = reportDetailAttributes;
        newReportDetailAttributes[index] = {...data, end_time: time};
        setReportDetailAttributes(newReportDetailAttributes);
        setEventCount(eventCount + 1);
        clearErrorMessage('endTimeError', index);
        checkTimeDifference(newReportDetailAttributes, index);
    };
    ...
    function checkTimeDifference(reportDetailAttributes, index) {
        let {start_time, end_time} = reportDetailAttributes[index];
        if (end_time === null || !end_time) {
            return false;
        }
        if (start_time === null) {
            alert('Please enter start time first');
            reportDetailAttributes[index] = {...reportDetailAttributes[index], end_time: null};
            setReportDetailAttributes(reportDetailAttributes);
            setEventCount(eventCount + 1);
            return false;
        }
        let seconds = parseInt((end_time.getTime() - start_time.getTime()) / 1000);
        let minutes = parseInt(seconds / 60);
        if (minutes > 60) {
            alert('Time of one task should not be greater than 1 hour. You can break the task into two or more tasks.');
            reportDetailAttributes[index] = {...reportDetailAttributes[index], end_time: null};
            setReportDetailAttributes(reportDetailAttributes);
            setEventCount(eventCount + 1);
            return false;
                ...
                <Card style={styles.cardStyle}>
                    {
                        reportDetailAttributes && reportDetailAttributes.map((data, index) => {
                            const {
                                new_task, projectKey, new_project, project_id, task, start_time, end_time, description,
                                errors: {projectError, taskError, startTimeError, endTimeError, descriptionError},
                            } = data;
                            return (
                                <View style={styles.newStatusReport} key={index}>
                                    {
                                        ...
                                        <>
                                            <Select value={project_id}
                                                    ...
                                                    style={[styles.selectStyle, projectError ? styles.error : {}]}
                                    ...
                                    <Row style={{marginTop: new_project ? 10 : -5}}>
                                        <Col style={{width: '85%'}}>
                                            <TextInput style={[styles.taskStyle, taskError ? styles.error : {}]}
                                                       placeholder={'Task'}
                                                       value={task}
                                                       ...
                                                       editable={!loading}
                                                       onChangeText={(value) => setTask(value, index)}/>
                                        </Col>
                                        <Col style={[styles.justifyContentCenter, {width: '15%'}]}>
                                            {
                                                new_task ?
                                                    <TouchableOpacity activeOpacity={1}
                                                                      style={[styles.justifyContentCenter, {minHeight: 20}]}
                                                                      onLongPress={() => duplicateTask(() => addNewDuplicateTask(projectKey, index))}
                                                                      onPress={() => addNewTask(projectKey)}>
                                                        <EnTypo size={30}
                                                                disabled={loading}
                                                    ...
                                                    <TouchableOpacity activeOpacity={1}
                                                                      ...
                                                                      onPress={() => confirm('Task', () => removeTask(index))}>
                                                        <EnTypo size={30}
                                                                style={styles.justifyContentCenter}
                                                                disabled={loading}
                                    ...
                                    <Row style={styles.borderRowBottom}>
                                        <FieldRequired show={taskError} marginTop={5}/>
                                    </Row>
                                    <Row>
                                        <Col size={1} style={styles.selectTimeColStyle}>
                                            <Text style={styles.selectTimeStyle}>
                                                Start Time
                                            </Text>
                                        </Col>
                                        <Col size={4} style={{marginRight: 10}}>
                                            <TimePicker onChange={(date) => setTimeStart(date, index)}
                                                        disabled={loading}
                                                        textStyle={startTimeError ? styles.error : {}}
                                                        name={'start_time'}
                                        ...
                                        <Col size={4} style={{marginRight: 5}}>
                                            <TimePicker onChange={(date) => setTimeEnd(date, index)}
                                                        defaultValue={start_time ?? new Date()}
                                                        textStyle={endTimeError ? styles.error : {}}
                                                        disabled={loading}
                                                        name={'end_time'}
                                                        value={end_time}/>
                                        </Col>
                                    ...
                                    <Row style={styles.borderRowBottom}>
                                        <Col size={1} style={styles.alignCenter}>
                                            <FieldRequired show={startTimeError}/>
                                        </Col>
                                        <Col size={1} style={styles.alignCenter}>
                                            <FieldRequired show={endTimeError}/>
                                        </Col>
                                    </Row>
                                    <Row>
...
function duplicateTask(onConfirm) {
    Alert.alert(
        'Add Task',
        `Are you sure you want to add duplicate task?`,
        [
            {
                text: 'Yes',
                onPress: onConfirm,
