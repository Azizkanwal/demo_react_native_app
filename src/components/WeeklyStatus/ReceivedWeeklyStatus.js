...
import {getReceivedWeeklyStatusReports, startLoader, stopLoader} from "../../redux/slices";
import {WeeklyStatusList} from "./WeeklyStatusList";
import Toast from "react-native-toast-message";

export const ReceivedWeeklyStatusScreen = (props: any) => {
    const { navigation } = props;
    const [loading, setLoading] = useState(false);
    const [search, setSearch] = useState("");
    const itemPerPage = 10;
    const dispatch = useAppDispatch();
    const {rows, count, currentPage} = useAppSelector((state) => state.statusReport.receivedWeeklyStatusReports);
    ...
    const onPageChange = async (page) => {
        ...
        await dispatch(getReceivedWeeklyStatusReports({page: page + 1, items: itemPerPage},renderResponseMessage));
            ...
            <ScrollView bounces={false}>
                <View style={styles.containerView}>
                    <Text style={styles.dailyStatusText}>
                        Weekly Status Received List
                    </Text>
                </View>
                <View style={styles.dailyStatusList}>
                    <WeeklyStatusList type={'received'}
